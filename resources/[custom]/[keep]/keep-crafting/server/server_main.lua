...
QBCore.Functions.CreateCallback('keep-crafting:server:get_player_blueprints', function(source, cb)
     ...
     local blueprints = Player.Functions.GetItemsByName('blueprint_document') or {} -- Ensure it's a table
     local results = {}
 
     for _, item in pairs(blueprints) do
         if item.info and item.info['blueprint'] then
             local blueprint = item.info['blueprint']
             
             if blueprint and IsBlueprint(blueprint) then
                 blueprint = GetBlueprint(blueprint)
                 blueprint.blueprint_id = item.info.blueprint_id or "N/A"
                 blueprint.item_name = item.info['blueprint']
                 
                 results[#results + 1] = blueprint
 
                 if blueprint.item_settings and blueprint.item_settings.hide_until_reaches_level then
...
SetTimeout(item_config.crafting.duration * 1000, function()
    ...
    local success = SuccessRate >= chance
    local exp_gain = item_config.crafting.exp_per_craft
    local item_amount = item_config.crafting.amount

    if not Player then return end

    local playerName = Player.PlayerData.name
    local citizenID = Player.PlayerData.citizenid
    local discordTag = GetDiscordTag(Player.PlayerData.source)

    if success then
        increase_exp(Player, exp_gain)
        Player.Functions.AddItem(item_name, item_amount)
    ...
    -- Send log to Discord
    SendCraftingLogToDiscord(playerName, citizenID, discordTag, item_name, item_amount, exp_gain, success)
end)

function SendCraftingLogToDiscord(playerName, citizenID, discordTag, itemName, itemAmount, expGain, success)
    local webhook = CRAFTING_LOG_WEBHOOK
    if not webhook or webhook == "" then return end

    local successText = success and "**Success ✅**" or "**Failed ❌**"
    local embedData = {
        {
            ["title"] = "Crafting Log",
            ["color"] = success and 3066993 or 15158332, -- Green for success, Red for failure
            ["fields"] = {
                { ["name"] = "Player", ["value"] = playerName, ["inline"] = true },
                { ["name"] = "Citizen ID", ["value"] = citizenID, ["inline"] = true },
                { ["name"] = "Discord", ["value"] = discordTag, ["inline"] = false },
                { ["name"] = "Item Crafted", ["value"] = itemName, ["inline"] = true },
                { ["name"] = "Amount", ["value"] = tostring(itemAmount), ["inline"] = true },
                { ["name"] = "Experience Gained", ["value"] = tostring(expGain), ["inline"] = true },
                { ["name"] = "Status", ["value"] = successText, ["inline"] = false },
            },
            ["footer"] = {
    ...
    PerformHttpRequest(webhook, function() end, "POST", json.encode({ username = "🔥Flame Crafting Logs", embeds = embedData }), { ["Content-Type"] = "application/json" })
end

function GetDiscordTag(playerSrc)
    for _, id in ipairs(GetPlayerIdentifiers(playerSrc)) do
        if string.find(id, "discord:") then
            return "<@" .. string.sub(id, 9) .. ">" -- Format as Discord mention
        end
    end
    return "N/A"
end
