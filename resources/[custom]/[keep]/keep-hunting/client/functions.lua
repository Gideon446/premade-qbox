...
function spawn_seller_npc()
    ...
    for _, spot in pairs(Config.SellSpots) do
        ...
        exports.ox_target:addBoxZone({
            coords = c,
            size = vec3(0.5, 0.5, 1.8),
            rotation = spot.SellerNpc.coords.w,
            debug = false,
            drawSprite = false,
            options = {
                {
                    ...
                    onSelect = function()
                        TriggerEvent('keep-hunting:client:sellREQ')
                    end
                }
            },
            distance = 1.5
        ...
        exports.ox_target:addModel(v.model, {
            {
                ...
                canInteract = function(entity, distance, coords, name, bone)
                    return entity and IsEntityDead(entity) and not IsPedAPlayer(entity)
                end,
                onSelect = function(data)
                    local entity = data.entity
                    if not IsPedAPlayer(entity) and IsEntityDead(entity) then
