...
window.addEventListener('message', function(event) {
    ...
    if (event.data.action == "openlist") {
        ...
        // If transitioning from cooldown to safe, add the completion log
        if (type === "safe" && $("#prioText").attr("data-status") === "cooldown") {
            addLogEntry("System", "safe", Math.floor(Date.now() / 1000));
        }
