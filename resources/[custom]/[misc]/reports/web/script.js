...
function addReportItem(reportData) {
  ...
  if (resolveButton) {
      resolveButton.addEventListener('click', function() {
          ...
          statusLabel.className = 'status-label text-xs px-2 py-1 rounded text-green-400 bg-green-900 bg-opacity-20';
					fetch('https://reports/changeReportStatus', {
						method: 'POST',
						headers: {
  ...
  if (rejectButton) {
	...
	if (teleportButton) {
		teleportButton.addEventListener('click', function() {
			fetch('https://reports/teleportPlayer', {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json'
				},
	...
	if (bringButton) {
		bringButton.addEventListener('click', function() {
			fetch('https://reports/bringPlayer', {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json'
				},
...
const closeUi = () => {
    ...
    fetch('https://reports/close')
}

const Notify = (message, type) => {
    fetch('https://reports/notify', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
const submitReport = () => {
    ...
    fetch('https://reports/submitReport', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
