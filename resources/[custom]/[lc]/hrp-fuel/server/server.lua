...
function checkVersion()
    CreateThread(function()
        ...
        while not connected and attempts < 3 do
            ...
            PerformHttpRequest("https://raw.githubusercontent.com/LeonardoSoares98/hrp-fuel/main/version", function(errorCode, resultData)
                if errorCode == 200 and resultData then
                    ...
                    if Utils.Math.checkIfCurrentVersionisOutdated(latest_version, version) then
                        ...
                        print("^4["..GetCurrentResourceName().."] An update is available, download it in https://github.com/LeonardoSoares98/hrp-fuel/releases/latest/download/hrp-fuel.zip^7 ^3[v"..api_response.latest_version.."]^7")
                    else
...
RegisterServerEvent("hrp-fuel:serverOpenUI")
AddEventHandler("hrp-fuel:serverOpenUI",function(isElectric, pumpModel, vehicleFuel, vehicleTankSize, vehiclePlate)
    local source = source
...
RegisterServerEvent("hrp-fuel:confirmRefuel")
AddEventHandler("hrp-fuel:confirmRefuel",function(data)
    local source = source
    Wrapper(source,function(user_id)
        if not data or data.fuelAmount <= 0 or not isFuelTypeValid(data.selectedFuelType) or not isPaymentMethodValid(data.paymentMethod) then
            TriggerClientEvent("hrp-fuel:Notify", source, "error", Utils.translate('invalid_value'))
            return
        end
        ...
        if Utils.Framework.getPlayerAccountMoney(source, Config.Accounts[data.paymentMethod]) < finalPrice then
            TriggerClientEvent("hrp-fuel:Notify", source, "error", Utils.translate('not_enough_money'):format(Utils.numberFormat(finalPrice)))
            return
        end

        if not removeStockFromStation(gasStationId, finalPrice, data.fuelAmount, data.selectedFuelType, false) then
            TriggerClientEvent("hrp-fuel:Notify", source, "error", Utils.translate('not_enough_stock'))
            return
        end
        ...
        TriggerClientEvent("hrp-fuel:getPumpNozzle", source, data.fuelAmount, data.selectedFuelType)
        TriggerClientEvent("hrp-fuel:Notify", source, "success", Utils.translate('refuel_paid'):format(Utils.numberFormat(finalPrice)))
    end)
end)


RegisterServerEvent("hrp-fuel:returnNozzle")
AddEventHandler("hrp-fuel:returnNozzle",function(remainingFuel, isElectric)
    local source = source
    Wrapper(source,function(user_id)
        ...
        if isElectric then
            TriggerClientEvent("hrp-fuel:Notify", source, "success", Utils.translate('returned_charge'):format(Utils.Math.round(remainingFuel, 1), amountToReturn))
        else
            TriggerClientEvent("hrp-fuel:Notify", source, "success", Utils.translate('returned_fuel'):format(Utils.Math.round(remainingFuel, 1), amountToReturn))
        end
...
RegisterServerEvent("hrp-fuel:confirmJerryCanPurchase")
AddEventHandler("hrp-fuel:confirmJerryCanPurchase",function(data)
    ...
    Wrapper(source,function(user_id)
        ...
        if Utils.Framework.getPlayerAccountMoney(source, Config.Accounts[data.paymentMethod]) < Config.JerryCan.price then
            TriggerClientEvent("hrp-fuel:Notify", source, "error", Utils.translate('not_enough_money'):format(Config.JerryCan.price))
            return
        end
        ...
        TriggerClientEvent("hrp-fuel:Notify", source, "success", Utils.translate('jerry_can_paid'):format(Config.JerryCan.price))
        TriggerClientEvent("hrp-fuel:closeUI", source, data.fuelAmount, data.selectedFuelType)
    end)
end)
...
function serverOpenUI(source, isElectric, pumpModel, gasStationId, vehicleFuel, vehicleTankSize, vehiclePlate)
    ...
    TriggerClientEvent("hrp-fuel:clientOpenUI", source, data)
end
...
Utils.Callback.RegisterServerCallback('hrp-fuel:getVehicleFuelType', function(source, cb, plate)
    cb(getVehicleFuelType(plate))
end)

RegisterServerEvent("hrp-fuel:setVehicleFuelType")
AddEventHandler("hrp-fuel:setVehicleFuelType",function(plate, fuelType)
...
function cacheplayerVehiclesFuelTypeType()
    ...
    print("^2[hrp-fuel] Fuel types successfully fetched from database^7")
end

function Wrapper(source,cb)
    if utils_outdated then
        TriggerClientEvent("hrp-fuel:Notify",source,"error","The script requires 'lc_utils' in version "..utils_required_version..", but you currently have version "..Utils.Version..". Please update your 'lc_utils' script to the latest version: https://github.com/LeonardoSoares98/lc_utils/releases/latest/download/lc_utils.zip")
        return
    end
...
Citizen.CreateThread(function()
    ...
    local versionFile = LoadResourceFile("hrp-fuel", "version")
    if versionFile then
        version = Utils.Math.trim(versionFile)
        print("^2[hrp-fuel] Loaded! Support discord: https://discord.gg/U5YDgbh ^3[v"..version..subversion.."]^7")
    else
        error("^1[hrp-fuel] Warning: Could not load the version file.^7")
    end
...
function checkScriptName()
    assert(GetCurrentResourceName() == "hrp-fuel", "^3The script name does not match the expected resource name. Please ensure that the current resource name is set to '^1hrp-fuel^7'.")
end
