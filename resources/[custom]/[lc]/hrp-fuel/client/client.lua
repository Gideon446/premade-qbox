...
function clientOpenUI(pump, pumpModel, isElectricPump)
    ...
    if closestVehicleToPump and #(playerCoords - GetEntityCoords(closestVehicleToPump)) < 5 then
        ...
        TriggerServerEvent("hrp-fuel:serverOpenUI", isElectricPump, pumpModel, vehicleDisplayFuelAmount, vehicleTankSize, vehiclePlate)
    else
        -- Allow the user to open the UI even without vehicles nearby
        TriggerServerEvent("hrp-fuel:serverOpenUI", isElectricPump, pumpModel)
    end
end
...
RegisterNetEvent('hrp-fuel:clientOpenUI')
AddEventHandler('hrp-fuel:clientOpenUI', function(data)
    loadNuiVariables()
...
RegisterNUICallback('post', function(body, cb)
    if cooldown == nil then
        ...
        elseif body.event == "changeVehicleFuelType" then
            ...
            TriggerServerEvent('hrp-fuel:'..body.event,body.data)
        end
        cb(200)
...
RegisterNetEvent('hrp-fuel:closeUI')
AddEventHandler('hrp-fuel:closeUI', function()
    closeUI()
end)
...
-- Alias LegacyFuel's exports to point to our hrp-fuel functions:
AddEventHandler('__cfx_export_LegacyFuel_SetFuel', function(setCB)
    -- Redirect LegacyFuel:SetFuel to use hrp-fuel's SetFuel function
    setCB(SetFuel)
end)

AddEventHandler('__cfx_export_LegacyFuel_GetFuel', function(setCB)
    -- Redirect LegacyFuel:GetFuel to use hrp-fuel's GetFuel function
    setCB(GetFuel)
end)
...
function getVehicleFuelTypeFromServer(vehicle)
    ...
    Utils.Callback.TriggerServerCallback('hrp-fuel:getVehicleFuelType', function(fuelType)
...
function changeVehicleFuelType(vehicle, fuelType)
    ...
    if vehicle and #(playerCoords - GetEntityCoords(vehicle)) < 5 then
        ...
        TriggerServerEvent("hrp-fuel:setVehicleFuelType", GetVehicleNumberPlateText(vehicle), fuelType)
    else
...
RegisterNetEvent('hrp-fuel:Notify')
AddEventHandler('hrp-fuel:Notify', function(type,message)
