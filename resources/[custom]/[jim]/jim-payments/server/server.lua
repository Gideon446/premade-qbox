...
QBCore.Commands.Add("emscharge", Loc[Config.Lan].command["charge"], {}, false, function(source) TriggerClientEvent("jim-payments:client:PolCharge", source) end)
	...
	if price > 0 then
		if not Config.FineJobConfirmation then
			...
			elseif Config.Banking == "qb-management" then
				...
				TriggerEvent("qbx-management:server:addAccountMoney", tostring(biller.PlayerData.job.name), (price - commission))
