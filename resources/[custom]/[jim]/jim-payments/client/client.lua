...
CreateThread(function()
	...
	exports.ox_target:addSphereZone({
		name = "JimBank",
		coords = vec3(Config.CashInLocation.x, Config.CashInLocation.y, Config.CashInLocation.z),
		radius = 2.0,
		debug = Config.Debug,
		options = {
			{
				event = "jim-payments:Tickets:Menu",
				icon = "fas fa-receipt",
				label = Loc[Config.Lan].target["cashin_boss"],
				job = jobroles
			},
			{
				event = "jim-payments:Tickets:Menu",
				icon = "fas fa-receipt",
				label = Loc[Config.Lan].target["cashin_gang"],
				gang = gangroles
			}
		},
		distance = 2.0
	...
	for k, v in pairs(Config.CustomCashRegisters) do
		for i = 1, #v do
			...
			exports.ox_target:addBoxZone({
				name = "CustomRegister: " .. k .. i,
				coords = vec3(v[i].coords.x, v[i].coords.y, v[i].coords.z),
				size = vec3(0.47, 0.34, 0.5), -- width, length, height (adjust height as needed)
				rotation = v[i].coords[4],
				debug = Config.Debug,
				minZ = v[i].coords.z - 0.1,
				maxZ = v[i].coords.z + 0.4,
				options = {
					{
						...
						job = job,
						gang = gang,
						image = "" -- Optional if not using images
					}
				},
				distance = 2.0
			})
...
RegisterNetEvent('jim-payments:client:PolCharge', function()
	...
	if dialog then
        ...
        if not inputs.citizen or not inputs.price then return end
		TriggerServerEvent('jim-payments:server:PolCharge', inputs.citizen, inputs.price)
    end
end)
