...
CreateThread(function()
	for location in pairs(Config.Locations["Centres"]) do local loc = Config.Locations["Centres"][location]
		if loc.Enable then
			...
			exports.ox_target:addBoxZone({
				coords = vec3(jobLoc.Enter.coords.x, jobLoc.Enter.coords.y, jobLoc.Enter.coords.z),
				size = vec3(jobLoc.Enter.w, jobLoc.Enter.d, 3.0), -- 3.0 height covers minZ/maxZ range
				...
				options = {
					{
						...
						event = "jim-recycle:TeleWareHouse",
						-- args = {
			...
			Targets[nameExit] =
			exports.ox_target:addBoxZone({
				coords = vec3(jobLoc.Exit.coords.x, jobLoc.Exit.coords.y, jobLoc.Exit.coords.z),
				size = vec3(jobLoc.Exit.w, jobLoc.Exit.d, 3.0), -- Covers minZ to maxZ
			...
			exports.ox_target:addBoxZone({
				...
				size = vec3(jobLoc.Duty.w, jobLoc.Duty.d, 1.0), -- Covers minZ to maxZ (±0.5 = 1.0 total height)
			...
			if jobLoc.Trade then
				for i = 1, #jobLoc.Trade do
					...
					Targets[nameTrade] =
					exports.ox_target:addBoxZone({
						coords = vec3(jobLoc.Trade[i].coords.x, jobLoc.Trade[i].coords.y, jobLoc.Trade[i].coords.z - 1),
						size = vec3(jobLoc.Trade[i].w, jobLoc.Trade[i].d, 2.0), -- z-1 to z+1 = 2.0 height
	...
	for i = 1, #Config.Locations["Recycle"] do local loc = Config.Locations["Recycle"][i]
		...
		Targets[nameSell] =
		exports.ox_target:addBoxZone({
			coords = vec3(loc.coords.x, loc.coords.y, loc.coords.z - 1),
			size = vec3(1.0, 1.0, 2.0), -- z-1 to z+1 = 2.0 height
			...
			options = {
				{
					...
					event = "jim-recycle:Selling:Menu",
					-- args = {
						Ped = Peds[nameSell]
					-- }
				}
	...
	for i = 1, #Config.Locations["BottleBanks"] do local loc = Config.Locations["BottleBanks"][i]
		...
		exports.ox_target:addBoxZone({
			coords = vec3(loc.coords.x, loc.coords.y, loc.coords.z - 1),
			size = vec3(1.0, 1.0, 2.0), -- Covers minZ to maxZ (z-1 to z+1)
			rotation = loc.coords.w,
			debug = Config.Debug,
			options = {
				{
...
function PickRandomPackage(Trolly)
	...
	exports.ox_target:addLocalEntity(randPackage, {
		{
			name = "pickup_package_" .. randPackage, -- make sure this is unique if you're spawning multiple
			icon = "fas fa-magnifying-glass",
			label = Loc[Config.Lan].target["search"],
			event = "jim-recycle:PickupPackage:Start",
			-- args = {
				Trolly = Trolly
			-- }
		}
...
RegisterNetEvent("jim-recycle:PickupPackage:Hold", function(data) local Ped = PlayerPedId()
	...
	exports.ox_target:addLocalEntity(TrollyProp, {
		{
			name = "dropoff_trolly_" .. TrollyProp, -- ensure unique name if needed
			icon = "fas fa-recycle",
			label = Loc[Config.Lan].target["drop_off"],
			event = "jim-recycle:PickupPackage:Finish",
			-- args = {
				Trolly = data.Trolly
			-- }
		}
...
RegisterNetEvent("jim-recycle:PickupPackage:Finish", function(data) local Ped = PlayerPedId()
	...
	if Targets["DropOff"] then exports.ox_target:removeEntity(TrollyProp, Loc[Config.Lan].target["drop_off"]) Targets["DropOff"] = nil end
	...
	-- toggleItem(true, "weapon_vintagepistol", 1)

	local chancePistol = Config.chancePistol
	local randomChance = math.random(1, 100)
	if chancePistol >= randomChance then
		toggleItem(true, "weapon_vintagepistol", 1)
		-- toggleItem(true, "pistol_ammo", 1)
	end
