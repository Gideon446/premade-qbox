...
RegisterNetEvent('jim-recycle:server:toggleItem', function(give, item, amount, newsrc)
	...
	if give == 0 or give == false then
		if HasItem(src, item, amount or 1) then -- check if you still have the item
			...
			TriggerClientEvent('qb-inventory:client:ItemBox', src, Core.Shared.Items[item], "remove", amount or 1) end
            ...
            '\n**Time:** '..currentTime.hour..":"..currentTime.min..":"..currentTime.sec..
            '\n**Item:** Removed '..QBCore.Shared.Items[item].label..' (x'..amount..')'
			
			TriggerEvent('qb-log:server:CreateLog', 'jimrecycle', 'Jim-Recycle', 'red', message, false)
		...
		else return dupeWarn(src, item) end -- if not boot the player
	else
		if Player.Functions.AddItem(item, amount or 1) then
			TriggerClientEvent('qb-inventory:client:ItemBox', src, Core.Shared.Items[item], "add", amount or 1)
            ...
            '\n**Date:** '..currentTime.year.."-"..currentTime.month.."-"..currentTime.day..
            '\n**Time:** '..currentTime.hour..":"..currentTime.min..":"..currentTime.sec..
            '\n**Item:** Added '..QBCore.Shared.Items[item].label..' (x'..amount..')'

			TriggerEvent('qb-log:server:CreateLog', 'jimrecycle', 'Jim-Recycle', 'white', message, false)
