...
Citizen.CreateThread(function()
    local blip = AddBlipForCoord(280.7188, -651.4319, 42.0186)
...
Citizen.CreateThread(function()
    local blip2 = AddBlipForCoord(-1818.35, -1209.95, 13.02)

    SetBlipSprite(blip2, 210)          
    ...
    QBCore.Functions.TriggerCallback('apartments:GetApartmentOffset', function(offset)
        if offset == nil or offset == 0 then
            QBCore.Functions.TriggerCallback('apartments:GetApartmentOffsetNewOffset', function(newoffset)
                ...
                local data = exports['cs-interior']:CreateApartmentFurnished(coords)
                Wait(100)
                HouseObj = data[1]
                POIOffsets = data[2]
                InApartment = true
                ...
                Wait(100)
                TriggerServerEvent('cs-apartments:server:SetInsideMeta', house, apartmentId, true, false)
            ...
            TriggerEvent('qb-weathersync:client:DisableSync')
            Wait(100)
            TriggerServerEvent('cs-apartments:server:SetInsideMeta', house, apartmentId, true, true)
            TriggerServerEvent('InteractSound_SV:PlayOnSource', 'houses_door_close', 0.1)
            TriggerServerEvent('apartments:server:setCurrentApartment', CurrentApartment)
        end
        ...
        if new ~= nil then
            if new then
                TriggerEvent('cs-interior:client:SetNewState', true)
            else
                TriggerEvent('cs-interior:client:SetNewState', false)
            end
        else
            TriggerEvent('cs-interior:client:SetNewState', false)
        end
...
local function LeaveApartment(house)
    TriggerServerEvent('InteractSound_SV:PlayOnSource', 'houses_door_open', 0.1)
    openHouseAnim()
    TriggerServerEvent('cs-apartments:returnBucket')
    DoScreenFadeOut(500)
    while not IsScreenFadedOut() do Wait(10) end
    exports['cs-interior']:DespawnInterior(HouseObj, function()
	    TriggerServerEvent("qb-weathersync:server:toggleFreezeTime", false)	
        ...
        TriggerServerEvent('cs-apartments:server:SetInsideMeta', CurrentApartment, false)
...
RegisterNetEvent('cs-apartments:client:LastLocationHouse', function(apartmentType, apartmentId)
    ...
    if CurrentApartment ~= nil then
        ...
        if not ox_inventory then
            ...
            if not ox_inventory:openInventory('stash', CurrentApartment) then
                TriggerServerEvent('cs-apartments:server:RegisterStash', CurrentApartment, Apartments.Locations[ClosestHouse].label)
