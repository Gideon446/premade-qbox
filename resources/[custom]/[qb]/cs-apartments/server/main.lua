...
-- Functions
-- RegisterCommand('migrateStashes', function()
--     -- Fetch stash items from the QB stash database table
--     local qbStash = MySQL.query.await('SELECT * FROM `stashitems`')  -- stash items table in QB
--     if not qbStash or #qbStash == 0 then
--         print("No stash items found for migration.")
--         return
--     end

--     -- Loop through each stash item and prepare it for OX Inventory
--     for k, v in pairs(qbStash) do
--         local items = {}
--         local stashName = v.stash
--         local decodedItems = json.decode(v.items)

--         -- Decode each item and adjust its structure for OX Inventory
--         for i, j in pairs(decodedItems) do
--             items[i] = j
--             items[i].metadata = items[i].info or {}
--             if items[i].info then 
--                 items[i].info = nil 
--             end
--             items[i].count = items[i].amount or 0
--             if items[i].amount then 
--                 items[i].amount = nil 
--             end
--         end

--         -- Check if the stash already exists in OX Inventory
--         local existing = MySQL.query.await('SELECT 1 FROM `ox_inventory` WHERE `name` = ?', {stashName})

--         if existing and #existing > 0 then
--             -- Update the existing entry
--             MySQL.update.await('UPDATE `ox_inventory` SET `data` = ? WHERE `name` = ?', {
--                 json.encode(items), 
--                 stashName
--             })
--             print("Updated existing stash: " .. stashName)
--         else
--             -- Insert the migrated stash data into the OX Inventory database table
--             MySQL.insert.await('INSERT INTO `ox_inventory` (owner, name, data) VALUES (?, ?, ?)', {
--                 '',  -- owner is empty as this is a stash
--                 stashName,
--                 json.encode(items)
--             })
...
--     print("Stash migration completed.")
-- end)
...
RegisterNetEvent('cs-apartments:server:SetInsideMeta', function(house, insideId, bool, isVisiting)
    local src = source
...
RegisterNetEvent('cs-apartments:returnBucket', function()
    local src = source
...
RegisterNetEvent('apartments:server:UpdateApartment', function(type, label)
   ...
   -- TriggerClientEvent("apartments:client:SetHomeBlip", src, type)
    if ox_inventory then
        ...
        if result[1] ~= nil then
            TriggerEvent('cs-apartments:server:RegisterStash', result[1].name, label) -- rename the stash name in case of apartment change
        end
    end
end)
...
if GetResourceState('ox_inventory') ~= 'missing' then
    ...
    RegisterNetEvent('cs-apartments:server:RegisterStash', function(currentApartmentId, currentApartmentLabel)
