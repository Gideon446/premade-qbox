...
local function saveVehicle()
    ...
    TriggerServerEvent('nrp-customs:server:updateVehicle', myCar)
end
...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("nrp-customs:server:attemptPurchase", type, upgradeLevel, CustomsData.location)
...
function EnterLocation(override)
    ...
    QBCore.Functions.TriggerCallback('aj-vehicletuning:server:IsMechanicAvailable', function(currentMechanics)
...
function GetLocations()
    QBCore.Functions.TriggerCallback("nrp-customs:server:GetLocations", function(locations)
    ...
    if next(CustomsData) then
        CreateThread(function()
            while next(CustomsData) and not isPlyInBennys do
                if IsControlJustReleased(0, 57) and CheckRestrictions(CustomsData.location) then EnterLocation() return end
                Wait(3)
            end
        end)
    end
            ...
            if Config.UseRadial then
                if not radialMenuItemId then
                    radialMenuItemId = exports['qb-radialmenu']:AddOption({
                        ...
                        icon = 'wrench',
                        type = 'client',
                        event = 'nrp-customs:client:EnterCustoms',
                        shouldClose = true
                    })
                end
            else
...
RegisterNetEvent('nrp-customs:client:UpdateLocation', function(location, type, key, value)
    Config.Locations[location][type][key] = value
end)

RegisterNetEvent("nrp-customs:client:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
    QBCore.Functions.Notify("Purchase Successful")
end)

RegisterNetEvent("nrp-customs:client:purchaseFailed", function()
    isPurchaseSuccessful = false
    attemptingPurchase = false
    QBCore.Functions.Notify("Not enough money", "error")
end)

RegisterNetEvent('nrp-customs:client:EnterCustoms', function(override)
